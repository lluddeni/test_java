...
--#Silent# HOST&SCRIPT_LA more lms_license_agreement.txt

-- PROMT FOR LICENSE AGREEMENT ACCEPTANCE
--#Silent# DEFINE LANSWER=N
DEFINE LANSWER=Y --#Silent#
SET TERMOUT ON
--#Silent# ACCEPT&SCRIPT_LA LANSWER FORMAT A1 PROMPT 'Accept License Agreement? (y\n): '

HOST&SCRIPT_LA rm   lms_license_agreement.txt
HOST&SCRIPT_LA del  lms_license_agreement.txt
...
BEGIN
  if    '&SEL_VER' = 'UNKNOWN' then
    dbms_output.put_line('ERROR: Current database user ' || USER || ' does not have the privilege to query V$INSTANCE. Cannot continue.');
    dbms_output.put_line('Exiting...');
    :MIS_PRIV := 'X';
    return;
  elsif '&SEL_PRIV' = 'N' then
    dbms_output.put_line('WARNING: Cannot check privileges. Continuing anyway. Errors expected.');
    dbms_output.put_line('.');
    :MIS_PRIV := 'N';
    return;
  elsif '&SEL_VER' = '8' and '&SEL_O7' = 'UNKNOWN' then
    dbms_output.put_line('WARNING: Current database user ' || USER || ' does NOT have the privilege to query V$PARAMETER. Continuing anyway. Errors expected.');
    SEL_DICT_REQUIRED := 'N';
  elsif '&SEL_VER' = '8' and '&SEL_O7' = 'FALSE' then
    dbms_output.put_line('WARNING: O7_DICTIONARY_ACCESSIBILITY=FALSE. Access to the data dictionary views requires explicit object privilege. Errors expected.');
    SEL_DICT_REQUIRED := 'N';
  ...
  loop
    ...
    begin
      ...
      if    MIS_AUTH_TEXT is not null and :MIS_PRIV = 'Y' then
        MIS_AUTH_TEXT := '!        It also has no authorization on ' || MIS_AUTH_TEXT || '.';
        MIS_AUTH := 'Y';
      elsif MIS_AUTH_TEXT is not null and :MIS_PRIV = 'N' then
        MIS_AUTH_TEXT := 'WARNING: Current database user ' || USER || ' does NOT have authorization on ' || MIS_AUTH_TEXT || '!';
        MIS_AUTH := 'Y';
       :MIS_PRIV := 'Y';
    ...
    exception
      when others then
        ERR_C := SQLCODE;
        if     ERR_C = -01031 and :MIS_PRIV = 'Y' then
          MIS_AUTH_TEXT := '!        It also has no authorization on Oracle Database Vault  realm.';
          MIS_AUTH := 'Y';
        elsif  ERR_C = -01031 and :MIS_PRIV = 'N' then
          MIS_AUTH_TEXT := 'WARNING: Current database user ' || USER || ' does NOT have authorization on Oracle Database Vault  realm!';
          MIS_AUTH := 'Y';
         :MIS_PRIV := 'Y';
        else
  ...
  if   :MIS_PRIV = 'Y' and MISSING_PRIVS is not null then
    dbms_output.put_line('WARNING: Current database user ' || USER || ' does NOT have ' || MISSING_PRIVS || '!');
  end if;
  if MIS_AUTH = 'Y' then
  ...
  if    '&SCRIPT_SI' is not null and :MIS_PRIV = 'Y' then
    dbms_output.put_line('RUNNING IN SILENT MODE. CONTINUING WITHOUT ANY PROMPT. ERRORS EXPECTED.');
  end if;
...
define OPTION_QUERY_COLS=NAME,VERSION,DETECTED_USAGES,TOTAL_SAMPLES,CURRENTLY_USED,FIRST_USAGE_DATE,LAST_USAGE_DATE,LAST_SAMPLE_DATE,SAMPLE_INTERVAL,DBID
define OCOUNT=-942
col OCOUNT new_val OCOUNT noprint
select ltrim(rtrim(to_char(count(*)))) as OCOUNT
  FROM DBA_FEATURE_USAGE_STATISTICS;
...
host echo select \'$PWD\' as PWD_, \'rm\' as RMDEL_, \'/\' as PSEP_ from dual where \'$PWD\' like \'%/%\'\; > psep.sql
...
-- #silent bug in Mobaxterm# host powershell.exe  -ExecutionPolicy Bypass -F "md5.ps1" "&&HOST_NAME._&&INSTANCE_NAME." >> &&SUMF 2>> fii_err.txt
host echo === sha1sum                                                >> &&SUMF
host /usr/bin/sha1sum  &&OUTF ReviewLite*.*                          >> &&SUMF 2>> fii_err.txt
-- #silent bug in Mobaxterm# host powershell.exe  -ExecutionPolicy Bypass -F "sha1.ps1" "&&HOST_NAME._&&INSTANCE_NAME." >> &&SUMF 2>> fii_err.txt
host echo ========================================================== >> &&SUMF
